...
LED_Profile_TypeDef LED_Profiles[12] = {  // the sequence is the priority from high to low
//Priority  acctive flag:          blink mode:                blink rate :           blink cnt:         color: 
  ...
  {8,       LED_RUN_STATE_STOPPED, LED_BLINK_MODE_CONTINUOUS, LED_BLINK_RATE_NORMAL, LED_BLINK_TIMES_1, LED_OFF,       LED_OFF        , LED_COLOR_BLUE},  // LED_Profile_COLD: Blinks Blue
  {9,       LED_RUN_STATE_STOPPED, LED_BLINK_MODE_CONTINUOUS, LED_BLINK_RATE_NORMAL, LED_BLINK_TIMES_1, LED_COLOR_RED, LED_COLOR_GREEN, LED_OFF},	    // LED_Profile_Heating: Blinks orange
  {10,      LED_RUN_STATE_STOPPED, LED_BLINK_MODE_CONTINUOUS, LED_BLINK_RATE_NORMAL, LED_SOLID_ON,      LED_COLOR_RED, LED_OFF,         LED_OFF},	    // LED_Profile_Fast_Charge: Solid red
  {11,      LED_RUN_STATE_STOPPED, LED_BLINK_MODE_CONTINUOUS, LED_BLINK_RATE_NORMAL, LED_SOLID_ON,      LED_OFF,       LED_COLOR_GREEN, LED_OFF},	    // LED_Profile_Full_Float_Charge: Solid green
};
...
// set speicified color blink pattern
...
// set  color 
  ...
  if( Current_Priority == 9 )                                               // this is for heating display color : orange, as the green LED is too bright than others, so dim it down.
  {
...
// set blink LED turn on time period according to the blink speed	and number of times:
 ...
 // this is slow blinking: long turn on time and off time period 
  ...
  else if(Blink_speed == LED_BLINK_RATE_NORMAL )     // for normal blinking speed                            
  {
...
void LED_task(LED_Display_TypeDef Display_Type, FunctionalState Enable) // enable the LED display accoding to conditions
{
   switch (Display_Type)
   {
  	case IR_Signal:  // priority 0
  	{
  	  if (Enable !=RESET) {	LED_Profiles[IR_Signal].Run_state = LED_RUN_STATE_RUNNING; }	 			 // make sure turn it off when leave this mode		
  	  else {LED_Profiles[IR_Signal].Run_state = LED_RUN_STATE_STOPPED;}	
    	
  	  break;
  	} 
  	 
  	case Low_Capacity: // priority 1
  	{
      if (Enable !=RESET) {LED_Profiles[Low_Capacity].Run_state = LED_RUN_STATE_RUNNING; }
  	 else {LED_Profiles[Low_Capacity].Run_state = LED_RUN_STATE_STOPPED;}
  	 break;
  	}
  	
  	case _90M_TEST:   // priority 2
  	{
  	  if (Enable !=RESET) {	LED_Profiles[_90M_TEST].Run_state = LED_RUN_STATE_RUNNING; }	 			 // make sure turn it off when leave this mode		
  	  else {LED_Profiles[_90M_TEST].Run_state = LED_RUN_STATE_STOPPED;}	
    	
  	  break;
  	}
  
  	case _30s_TEST:   // priority 3                                       // make sure turn it off when leave this mode
  	{
  	  if (Enable !=RESET) {	LED_Profiles[_30s_TEST].Run_state = LED_RUN_STATE_RUNNING; }	 			 // make sure turn it off when leave this mode		
  	  else {LED_Profiles[_30s_TEST].Run_state = LED_RUN_STATE_STOPPED;}						
  				
  	  break;
  	}
  
     case Batt_Disconnect:   // priority 4
  	{
       if (Enable !=RESET) 
	  {
	    LED_Profiles[Batt_Disconnect].Run_state = LED_RUN_STATE_RUNNING;
	    LED_Profiles[Fast_Charge].Run_state = LED_RUN_STATE_STOPPED; 
	    LED_Profiles[Full_Float_Charge].Run_state = LED_RUN_STATE_STOPPED;
	    
       }
  	  else {LED_Profiles[Batt_Disconnect].Run_state = LED_RUN_STATE_STOPPED;}
  	  break;
  	}
 
     case Charger_Fail:     // priority 5
  	{
  	  if (Enable !=RESET) {LED_Profiles[Charger_Fail].Run_state = LED_RUN_STATE_RUNNING; }
  	  else {LED_Profiles[Charger_Fail].Run_state = LED_RUN_STATE_STOPPED;}
  
  	  break;
  	}
 
     case Lamp_Fail:        // priority 6
  	{
  	  if (Enable !=RESET) {LED_Profiles[Lamp_Fail].Run_state = LED_RUN_STATE_RUNNING; }
  	  else {LED_Profiles[Lamp_Fail].Run_state = LED_RUN_STATE_STOPPED;}
  	  break;
  	}
  
  	case Emergency:      // priority 7
  	{
  	  if (Enable !=RESET) {	LED_Profiles[Emergency].Run_state = LED_RUN_STATE_RUNNING;}	 			 // make sure turn it off when leave this mode		
  	  else {LED_Profiles[Emergency].Run_state = LED_RUN_STATE_STOPPED;}	
      
       break;
  	}
  
     case Batt_Cold:        // priority 8
  	{
       if (Enable !=RESET) 
	  {
	    LED_Profiles[Batt_Cold].Run_state = LED_RUN_STATE_RUNNING; 
	    LED_Profiles[Heating].Run_state = LED_RUN_STATE_STOPPED;
	  }
  	  else {LED_Profiles[Batt_Cold].Run_state = LED_RUN_STATE_STOPPED;}
  	  break;
  	}

  	case Heating:        // priority 9
  	{
  	  if (Enable !=RESET) 
	  {	
	    LED_Profiles[Heating].Run_state = LED_RUN_STATE_RUNNING; 
	    LED_Profiles[Batt_Cold].Run_state = LED_RUN_STATE_STOPPED; 
	  }	 			                                                   		
  	  else {LED_Profiles[Heating].Run_state = LED_RUN_STATE_STOPPED;}					
  		
  	  break;
  	}
  	
  	case Fast_Charge:    // priority 10
  	{
  	  if (Enable !=RESET) 
	  {
	    LED_Profiles[Fast_Charge].Run_state = LED_RUN_STATE_RUNNING; 
	    LED_Profiles[Full_Float_Charge].Run_state = LED_RUN_STATE_STOPPED;
	    LED_Profiles[Batt_Disconnect].Run_state   = LED_RUN_STATE_STOPPED;
	  }
  	  else {LED_Profiles[Fast_Charge].Run_state = LED_RUN_STATE_STOPPED;}
  		
  	  break;
  	}
  	
    case Full_Float_Charge:   // priority 11
  	{
  	  if (Enable !=RESET) 
	  {
	    LED_Profiles[Full_Float_Charge].Run_state = LED_RUN_STATE_RUNNING;
	    LED_Profiles[Fast_Charge].Run_state = LED_RUN_STATE_STOPPED;
	    LED_Profiles[Batt_Disconnect].Run_state   = LED_RUN_STATE_STOPPED;
	  }
  	  else {LED_Profiles[Full_Float_Charge].Run_state = LED_RUN_STATE_STOPPED;}
   ...
   for (Priority_ID =0; Priority_ID < 12; Priority_ID++)                               // find the highest priority profile ID is in active to run 
   {
