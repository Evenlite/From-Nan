...
extern uint32_t Time_delay_StartTime;
...
uint8_t Zero_Cross_Task(void)
{
   if( System_State_Machine == EMERGENCY )
   {
	...
	{
//	  HAL_GPIO_WritePin(Test_Prob1_GPIO_Port,Test_Prob1_Pin,GPIO_PIN_RESET);
	  return AC_Good;
	  
     }
     else if ((Duty_Cycle >= 0) && (Duty_Cycle <= AC_FailVoltage)) // 37.6 measured zero-cross duty cycle value; based on 75% of 120V/AC - 90V
	{
//       HAL_GPIO_WritePin(Test_Prob1_GPIO_Port,Test_Prob1_Pin,GPIO_PIN_SET)	;
	  return AC_Fail;
	}
	else 
	  return Invalid;
...
uint16_t IR_CMD =0;
uint16_t IR_CMD_Last = 0;
uint16_t IR_CMD_Current =0;

uint16_t IR_shift_reg;
float sample_max;
float sample_min;
	...
	
{
 ...
 {	
// HAL_GPIO_WritePin(Test_Prob2_GPIO_Port,Test_Prob2_Pin,GPIO_PIN_SET);  //testing flag   
	 float  AC_Freq_temp;
	 static float Duty_temp =0;
			
	 if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2) // CC2 interrupt: Rising edge detected
	 {
	   if (AC_Cycle == 0)
	   {
		CC2a_val = HAL_TIM_ReadCapturedValue(&htim2, TIM_CHANNEL_2); // duty cycle cnt value: CC2a and CC2b are the captured value from two cycles ( occured in one AC cycle)
		AC_Cycle = 1;	
	   }
	   else if (AC_Cycle == 1)
	   {
      ...
      else if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)  // CC1 interrupt: Falling edge detected
	 {
	   __HAL_TIM_SET_COUNTER(&htim2, 0);                  // clear the TIM_CNT for not to trigger the timeout update interrupt
...
// HAL_GPIO_WritePin(Test_Prob2_GPIO_Port,Test_Prob2_Pin,GPIO_PIN_RESET);  //testing flag					
 }			
...
//HAL_GPIO_WritePin(Test_Prob2_GPIO_Port,Test_Prob2_Pin,GPIO_PIN_SET);  //testing flag 	
   ...
   if  (IR_bit_cnt ==12)
   {
     ...
     IR_CMD_Current = IR_shift_reg&0x0FFF;         // 12bytes code is shifted and get last 12 bytes
//USBVcom_printf("IR_receiving:%d\r\n", IR_receiving);

     if (IR_receiving == 0 )                      // get and verify the two first packages are the same to issue the IR command
     {
 	  if (IR_CMD_Current == IR_CMD_Last)         // and verify the second code
	  {
//USBVcom_printf("IR_CMD2:%X\r\n\r\n", IR_CMD);	    
	    IR_receiving = 1;                       // matched, set the flag of receiving for decoding 
	    IR_CMD = IR_CMD_Current;                // load the corrent IR code
	    
	  }  
	  IR_CMD_Last = IR_CMD_Current;             // save the current IR code 
	  
//USBVcom_printf("IR_CMD1:%X\r\n\r\n", IR_CMD);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)  // overflow interrupt
{
  if (htim == &htim3) 
  {
//HAL_GPIO_WritePin(Test_Prob1_GPIO_Port,Test_Prob1_Pin,GPIO_PIN_SET);  //testing flag
    __HAL_TIM_CLEAR_IT(htim, TIM_IT_UPDATE);
    
    IR_receiving = 0;   // reset the flag to terminate the receiving 
    IR_CMD_Current =0;
    IR_CMD_Last = 0;
//HAL_GPIO_WritePin(Test_Prob1_GPIO_Port,Test_Prob1_Pin,GPIO_PIN_RESET);  //testing flag	        
  }
...
HAL_GPIO_WritePin(Test_Prob2_GPIO_Port,Test_Prob2_Pin,GPIO_PIN_SET);  //testing flag
    ...
    __HAL_TIM_CLEAR_IT(htim, TIM_IT_UPDATE);
    Zero_cross_err_cnt ++;
    if (Zero_cross_err_cnt == 5) // continue timeout twice to tigger the Duty cycle to be reset
    {
...
void IR_task(void)
{
  if (IR_CMD!= 0)  
  {
    switch (IR_CMD)
    {
      case IR_CMD_RESET:      //  the system reboot
	 {
	   IR_CMD =0;
	   if((System_State_Machine == STANDBY)||(System_State_Machine == DEMO))
		{
	 ...
	 case IR_CMD_SHORT_TEST:   // 30 second test command
	 {
	   ...
	   else if (System_State_Machine == DEMO)
        {
          __HAL_TIM_SetCompare(&htim4, LAMP_PWM_CH, 100);
          Remote_Lamp_EN(ENABLE);
...
*/			
	   ...
	   case IR_CMD_PROG_EVO_HEAD_0W:    // lamp head wattage is 0W
	   {
		IR_CMD = 0;
		if(System_State_Machine == STANDBY)
		{
  		  NV_Profile_Runtime.HeadWattage = 0;	
		  while (IR_receiving != 0){};
		  Flash_write();
		  LAMP_Init();	
		}
