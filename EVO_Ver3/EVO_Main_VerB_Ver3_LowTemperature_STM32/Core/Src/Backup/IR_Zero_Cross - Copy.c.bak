   ...
   if( System_State_Machine == EMERGENCY )
	    {
	     if ( (Duty_Cycle >= AC_GoodVoltage) && (Duty_Cycle < 100))  // 47.5  measured zero-cross duty cycle value; based on 85% of 120V/AC: 102V
	  	...
	  	{
	  	 if (( Duty_Cycle < AC_FailVoltage) && (Duty_Cycle >=0) )  // 32.8  measured zero-cross duty cycle value; based on 75% of 120V/AC - 90V
	  	  { 
	  		 return AC_Fail;
	  	  }
...
//		  HAL_GPIO_WritePin(Test_Prob_GPIO_Port,Test_Prob_Pin,GPIO_PIN_SET);  //testing flag	
	...
	*/		
		  ...
		  __HAL_TIM_CLEAR_IT(htim, TIM_IT_UPDATE);
			...
			memcpy (&sample_buf[0], &sample_buf[1], (slide_windows_size)*4); // shifting the samples, first data removed and leave the last for new data
			sample_buf[slide_windows_size - 1] = Duty_temp;
      
      sample_max = sample_buf[0];
      sample_min = sample_buf[0];			
			for (j=1; j<slide_windows_size; j++)
			{
			  if (sample_buf[j] > sample_max)
				  { sample_max = sample_buf[j];}
				if (sample_buf[j] < sample_min)
					{ sample_min = sample_buf[j];}
			}
			
			
			for ( i= 0; i < slide_windows_size; i++)
					{sum += sample_buf[i];}
			sum = sum - sample_max - sample_min;
		 

			
			Duty_Cycle = sum / (slide_windows_size-2); // calculate average 
		...
		case IR_CMD_LONG_TEST:   // 90 minute test command
		  {
				...
				if( System_State_Machine == STANDBY )
				{	
          if( ADC_DMA_VALUE[ADC_BATT_VOLTAGE] > V_Low_Capacity )
					 {
					   HAL_GPIO_WritePin(nCHARGE_SHUNT_GPIO_Port,nCHARGE_SHUNT_Pin,GPIO_PIN_SET);  // disable shunt resister
					   Lamp_ctrl(ENABLE, NV_Profile_Runtime.HeadWattage);
					   Remote_Lamp_EN(ENABLE);
...
//					   Flash_write();
       ...
       if(System_State_Machine == STANDBY) 
		...
		case IR_CMD_ENTER_EOL_TEST_MODE:     // the factory final test mode
		  {
		    IR_CMD = 0;
		//	  time_stamp_start = HAL_GetTick();
				
		//		System_State_Machine = EOL;
			break;
			}	
